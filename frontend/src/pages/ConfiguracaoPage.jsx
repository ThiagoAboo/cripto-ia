...
function valueOr(defaultValue, value) {
...
export default function ConfiguracaoPage({ ctx }) {
  ...
  const paperConfig = draftConfig.execution?.paper || {};

  return (
    <div className="page-stack">
      <Section
        title="Configuração operacional"
        subtitle="Ajuste como o sistema opera, executa ordens, calcula taxas e protege o capital antes de deixar a automação rodar."
        actions={
          <button className="button" disabled={saving} onClick={handleSaveConfig}>
      ...
      >
        <div className="grid three-columns">
          <div className="page-stack">
            <h3 className="section-subtitle">Trading e universo operado</h3>
          ...
          <div className="page-stack">
            <h3 className="section-subtitle">Risco, execução e taxa</h3>
            <div className="form-grid">
              ...
              <ConfigField label="Capital inicial simulado"><input type="number" value={paperConfig.initialCapital} onChange={(e) => handleNumberChange('execution.paper.initialCapital', e.target.value)} /></ConfigField>
              <ConfigField label="Tamanho da ordem (%)"><input type="number" step="0.1" value={paperConfig.orderSizePct} onChange={(e) => handleNumberChange('execution.paper.orderSizePct', e.target.value)} /></ConfigField>
              <ConfigField label="Ordem mínima"><input type="number" step="0.01" value={paperConfig.minOrderNotional} onChange={(e) => handleNumberChange('execution.paper.minOrderNotional', e.target.value)} /></ConfigField>
              <ConfigField label="Taxa padrão Binance (%)" hint="Taxa usada quando não houver BNB elegível para desconto."><input type="number" step="0.01" value={paperConfig.feePct} onChange={(e) => handleNumberChange('execution.paper.feePct', e.target.value)} /></ConfigField>
              <ConfigField label="Taxa com BNB (%)" hint="Taxa reduzida a aplicar quando houver saldo de BNB disponível para pagar fees."><input type="number" step="0.01" value={valueOr(0.075, paperConfig.bnbFeePct)} onChange={(e) => handleNumberChange('execution.paper.bnbFeePct', e.target.value)} /></ConfigField>
              <ConfigField label="Reserva mínima de BNB" hint="Quando houver posição em BNB, evita venda abaixo desse saldo reservado para fees."><input type="number" step="0.0001" value={valueOr(0.05, paperConfig.minBnbReserveQty)} onChange={(e) => handleNumberChange('execution.paper.minBnbReserveQty', e.target.value)} /></ConfigField>
              <ConfigField label="Usar BNB para desconto da taxa"><input type="checkbox" checked={Boolean(valueOr(true, paperConfig.useBnbFeeDiscount))} onChange={(e) => handleCheckboxChange('execution.paper.useBnbFeeDiscount', e.target.checked)} /></ConfigField>
              <ConfigField label="Slippage (%)"><input type="number" step="0.01" value={paperConfig.slippagePct} onChange={(e) => handleNumberChange('execution.paper.slippagePct', e.target.value)} /></ConfigField>
      ...
      <div className="grid two-columns">
        <Section title="Histórico de configuração" subtitle="Versões anteriores para auditoria, comparação e rollback assistido.">
        ...
        <Section title="Auditoria de configuração" subtitle="Registro das alterações mais recentes e de quem solicitou cada mudança.">
          <div className="list-stack compact-scroll">
